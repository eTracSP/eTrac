            ...
            success: function (Data) {
                if (Data.length > 0) {
                    ...
                    for (i = 0; (i < Data.length) ; i++) {
                        ...
                        var Progress = "";
                        //if (AssignTime == "") {
                        //    HtmlString += '<li><span class="jTimer2" style="display: none">' + Data[i].EndTime + '</span>';
                        //} else {
                        //    var ass_tim = new Date(AssignTime);
                        //    var stat_tim = new Date(StartTime);
                        //    var seconds = ass_tim.getSeconds();
...
//Description :- Calculation and NaN issu with comments
    function myTimer2() {
        ...
        $('.jTimer1').each(function () {
            ...
            var txtid = $(this);
            var txtval = $(this).html();//Start Time
            var timeDiff = $(this).prev().html();//End Time

            if (timeDiff == undefined || timeDiff == "" || timeDiff == null || timeDiff == '') {
                $(txtid).next().css("color", "Green");
                $(txtid).next().html('No Limit');
                $(txtid).parents(".graybox").find(".progressbar").hide();
            }
            else {
                var spmmm = $(this).next();//Dummy Field
                var startdate = new Date(txtval).getTime();//Start Time convert to second
                var temp = moment().tz("America/Mexico_City").format('MM/DD/YYYY HH:mm:ss');//This is for according to 108 timezone date
                //var temp = moment().tz("US/Eastern").format('MM/DD/YYYY HH:mm:ss');//This is for according to client's timezone date
                var current_date = new Date(temp).getTime();//Current Time convert to second
                var target_date = new Date(timeDiff).getTime();//End Time convert to second
                ...
                // var Prog_Time = (seconds_Passed + seconds_left )/ (100*10);
                var Prog_Time = seconds_Passed / (seconds_Passed + seconds_left) * 100; 
                ...
                if (current_date > target_date) {
                    if (target_date == 0) {
                        $(txtid).next().css('color', 'green');
                        $(txtid).next().html('No Limit');
                        $(txtid).parent('td').next('.tdTimer').find(".tdbtnclass").show();
                        $(txtid).parents(".tdTimer").find(".tdbtnclass").css("display", "none");
                        $(txtid).parents(".graybox").find(".progressbar").hide();
                    }
                    else {
                        $(txtid).next().css('color', 'red');
                        $(txtid).next().html('Over Limit');
                        $(txtid).parent('td').next('.tdTimer').find(".tdbtnclass").show();
                        $(txtid).parents(".tdTimer").find(".tdbtnclass").css("display", "none");
                        $(txtid).parents(".graybox").find(".progressbar").hide();
                    }
                    ...
                    $(txtid).next().css('color', 'red');
                    // $(txtid).next().html(days + "D " + hours + "hr " + minutes + "m " + seconds + "s");
                    $(txtid).next().attr("data", days + "D " + hours + "hr " + minutes + "m " + seconds + "s");
                    //$(txtid).next().html(Pdays + "D " + Phours + "hr " + Pminutes + "m " + Pseconds + "s");
                    $(txtid).parent('td').next('.tdTimer').find(".tdbtnclass").show();
                    $(txtid).parents(".tdTimer").find(".tdbtnclass").css("display", "none");
                    $(txtid).parents(".graybox").find(".progressbar").show();
                }
                ...
                if (Prog_Time > 100) {
                    var ExtraTime = parseInt(Prog_Time) - 100;
                    Prog_Time = 100;
                    $(txtid).siblings("div").find('.bar').removeClass("[class^='color']");
                    $(txtid).siblings("div").find('.bar').addClass("color3"); //Color3 is Indicating Red
                }
                if (Prog_Time > 80 && Prog_Time < 99) {
                    $(txtid).siblings("div").find('.bar').removeClass("[class^='color']");
                    $(txtid).siblings("div").find('.bar').addClass("color2"); //color2 is indicating Yellow
                }
                ...
                $(txtid).siblings("div").attr('data-perc', Prog_Time);
                var t = $(txtid).siblings("div"),//.find(".progressbar"),
