...
var deleteURL = 'Vehicle/DeleteVehicle/';
var generateQRC = "Vehicle/_GenerateQRCForVehicle/";
...
$(function () {
    ...
    $("#tbl_QRCList").jqGrid({
        ...
        //colNames: ['Vehicle Identification No', 'Vendor', 'License No', 'Insurance Plan', 'Email', 'Driver Name', 'Actions', ],
        colNames: ['Vehicle Identification No', 'Vendor', 'License No', 'Email', 'BusinessNo', 'Driver Name', 'LocationName', 'Actions'],
        colModel: [{ name: 'VehicleIdentificationNo', width: 80, sortable: true },
                  { name: 'Vendor', width: 80, sortable: true },
                  { name: 'LicenseNo', width: 50, sortable: false },
                  //{ name: 'InsurancePlan', width: 80, sortable: false },
                  { name: 'Email', width: 50, sortable: true },
                  { name: 'BusinessNo', width: 60, sortable: true },
                  { name: 'DriverName', width: 50, sortable: true },
        ...
        gridComplete: function () {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                jQuery("#tbl_QRCList").jqGrid('setRowData', ids[i], { act: be + de + qrc });
            }
...
function gridReload() {
    ...
    jQuery("#tbl_QRCList").jqGrid('setGridParam', { url: $_HostPrefix + QRCurl + "?SearchText=" + txtSearch.trim() + "&statusType=" + statusType + "&LocationID="+ $_locationId, page: 1 }).trigger("reloadGrid");
}
$(".EditRecord").live("click", function (event) {
    var id = $(this).attr("Id");
    window.location.href = $_HostPrefix + editurl + '?qr=' + id;
...
$("#QRCGenerate").live("click", function (event) {
    ...
    var rowData = jQuery("#tbl_QRCList").getRowData(id);
   ...
   var LocationName = $("#drp_MasterLocation option[value='" + $_locationId + "']").text();
    $.ajax({
        ...
        success: function (result) {
            ...
            console.log('Content/Images/VehicleImages/' + imgVehicleImg);
...
$(".deleteRecord").live("click", function (event) {
    ...
    showPopupRelativeMessage("Are you sure want to delete vehicle?", $(this), "Delete Item", function () {
        $.ajax({
            type: "POST",
            data: "{'id':'" + id + "'}",
            url: deleteURL,
