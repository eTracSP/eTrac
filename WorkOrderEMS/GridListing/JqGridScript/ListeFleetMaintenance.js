...
var QRCurl = 'eFleetMaintenance/GetListMaintenance';
var editurl = 'eFleetMaintenance/EditMaintenance/';
var deleteURL = 'eFleetMaintenance/DeleteMaintenance/';
var generateQRC = "eFleetMaintenance/_GenerateQRCForeFleetVehicle/";
...
$(function () {
    $("#tbl_MaintenanceList").jqGrid({
        ...
        width: 700,
        autowidth: true,
        colNames: ['Vehicle No', 'Maintenance Type', 'Maintenance Date', 'Issue Details', 'Days Out of Services', 'Driver Name', 'Total Cost', 'Miles', 'Notes', 'Actions'],
        colModel: [{ name: 'VehicleNumber', width: 30, sortable: true },
        { name: 'MaintenanceType', width: 40, sortable: true },
        { name: 'MaintenanceDate', width: 40, sortable: false },
        { name: 'RemainderMetricDesc', width: 40, sortable: true },
        { name: 'DaysOutOfServices', width: 40, sortable: true },
        ...
        { name: 'TotalCost', width: 30, sortable: false },
        { name: 'Miles', width: 20, sortable: false},
        { name: 'Note', width: 30, sortable: false},
        ...
        shrinkToFit: true,
        sortorder: 'asc',
        caption: "List of Maintenance",
        gridComplete: function () {
            var ids = jQuery("#tbl_MaintenanceList").jqGrid('getDataIDs');
            for (var i = 0; i < ids.length; i++) {
                ...
                jQuery("#tbl_MaintenanceList").jqGrid('setRowData', ids[i], { act: be + de + qrc }); //+ qrc 
            }
            //Added By Ashwajit Bansod on 22/09/2017 if record < 20 then pager become hide 
            if ($("#tbl_MaintenanceList").getGridParam("records") <= 20) {
                $("#divQRCListPager").hide();
            }
            else {
                $("#divQRCListPager").show();
            }
            if ($('#tbl_MaintenanceList').getGridParam('records') === 0) {
                $('#tbl_MaintenanceList tbody').append("<div style='padding: 6px; font-size: 12px;'>No records found.</div>");
            }
        },
    ...
    if ($("#tbl_MaintenanceList").getGridParam("records") > 20) {
        jQuery("#tbl_MaintenanceList").jqGrid('navGrid', '#divQRCListPager', { edit: false, add: false, del: false, search: false, edittext: "Edit" });
    }
});
var timeoutHnd;
var flAuto = true;
...
function gridReload() {
    ...
    jQuery("#tbl_MaintenanceList").jqGrid('setGridParam', { url: $_HostPrefix + QRCurl + "?SearchText=" + txtSearch.trim() + "&statusType=" + statusType + "&LocationID=" + $_locationId, page: 1 }).trigger("reloadGrid");
}
...
$("#QRCGenerate").live("click", function (event) {
    debugger
    var id = $(this).attr("data-value");
    var rowData = jQuery("#tbl_MaintenanceList").getRowData(id);
    var VehicleNumber = rowData['VehicleNumber'];
    var MaintenanceType = rowData['MaintenanceType'];
    var MaintenanceDate = rowData['MaintenanceDate'];
    var RemainderMetricDesc = rowData['RemainderMetricDesc'];
    var DaysOutOfServices = rowData['DaysOutOfServices'];
    var DriverName = rowData['DriverName'];
    var TotalCost = rowData['TotalCost'];
    var Miles = rowData['Miles']; 
    var Note = rowData['Note'];
   // var VehicleImage = rowData['VehicleImage'];
    $("#lblVehicleNumber").html(VehicleNumber);
    $("#lblMaintenanceType").html(MaintenanceType);
    $("#lblMaintenanceDate").html(MaintenanceDate);
    $("#lblRemainderMetricDesc").html(RemainderMetricDesc);
    $("#lblDaysOutOfServices").html(DaysOutOfServices);
    ...
    //}
    //$("#lblLocationName").html(LocationName);
    $('.modal-title').text("eFleet Maintenance Details");
    $("#myModalFOReFleetMaintenanceQRC").modal('show');
});
$(".deleteRecord").live("click", function (event) {
    debugger
    var id = $(this).attr("cid");
    bootbox.dialog({
        message: "are you sure you want to delete this Maintenance Report?",
        buttons: {
            success: {
                label: "delete",
                ...
                callback: function () {
                    $.ajax({
                        type: "post",
                        url: '../eFleetMaintenance/DeleteMaintenance' + '?MaintenanceID=' + id,
                        ...
                        success: function (data) {
                            ...
                            $('#tbl_MaintenanceList').trigger('reloadgrid');
                            gridReload();
                        },
                        error: function () {
                            alert("error:")
...
function PrintDivForMaintenanceDetails(DivId) {
    debugger
    _isPrintDone = false;
    if (!_isPrintDone) {
        //var divToPrint = document.getElementById('DivQRCIndex');
        ...
        popupWin.document.write("<style>img {height: 110px;width: 115px;}</style><html><body onload='window.print();'><div style='margin-left: 96px; margin-right: 100px; width: 420px;' class='row '><table id='DivMaintenanceDetailsIndex' style='width: 400px; border: 1px solid #0aa0cd; padding: 10px;'><tr><td valign='top' style='width: 210px;'><p></p><strong style='font-size: 16px;'> QRC Code No</strong> <br/>"
